...
class ImageProcessor:
    ...
    def __sobel_thresh(self, img,orient='x',min=0,max=255):
    ...
    def __luv_select(self, img, thresh=(0, 255)):
    ...
    def __lab_select(self, img, thresh=(0, 255)):
    ...
    def __hls_select(self, img,channel='s',thresh=(0, 255)):
    ...
    def __rgb_select(self, img,thresh=(0,255)):
        ch=img[:,:,2]
    ...
    def __thresholding(self, img):
        ...
        hls_thresh = self.__hls_select(img, thresh=(40, 60))
        lab_thresh = self.__lab_select(img, thresh=(140, 220))
        luv_thresh = self.__luv_select(img, thresh=(170, 255))
        threshholded = np.zeros_like(hls_thresh)
        threshholded[((hls_thresh == 1) & (lab_thresh == 1)) & (luv_thresh==1)]=255
        return threshholded

    def decide_way(self, img):
        img=cv2.GaussianBlur(img,(5,5),0)
        blur=self.__thresholding(img)
